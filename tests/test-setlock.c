...
static int check(char const *binary, char const *self, char const *file,
                 char const *dest, char const *mode, int timed)
{
  ...
  if (!holder)
  {
    ...
    alarm(5);
    if (dest && timed)
      execl(binary, binary, mode, "-t", "50", "-d", dest, file, self, "--holder", dest, (char *)0);
    else if (dest)
      execl(binary, binary, mode, "-d", dest, file, self, "--holder", dest, (char *)0);
    else if (timed)
      execl(binary, binary, mode, "-t", "50", file, self, "--holder", (char *)0);
    else
  ...
  printf("%s %s descriptor=%s acquisition=%s: exclusion, timeout, release\n",
         failed ? "FAIL" : "PASS", mode, dest ? dest : "default", timed ? "timed" : "blocking");
  return failed;
}
...
int main(int argc, char **argv)
{
  ...
  for (unsigned int i = 0; i < sizeof destinations / sizeof destinations[0]; i++)
  {
    ...
    {
      failed += check(argv[1], argv[0], file, destinations[i], "-w", timed);
      failed += check(argv[1], argv[0], file, destinations[i], "-r", timed);
    }
  }
